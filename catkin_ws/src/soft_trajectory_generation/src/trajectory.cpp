...
void Trajectory::calculateSpline(Point point, Point point_prev){
    ...
    float vf_th = point.d_yawf;
    float ai_th = point_prev.d_d_yawf;
    float af_th = point.d_d_yawf;
    splineParameters(tf, sf_x-s0_x, vi_x, vf_x, ai_x, af_x, ax);
    splineParameters(tf, sf_y-s0_y, vi_y, vf_y, ai_y, af_y, ay);
    splineParameters(tf, sf_th-s0_th, vi_th, vf_th, ai_th, af_th, ath);
    s0x = s0_x;
    s0y = s0_y;
    s0th = s0_th;
}
...
float Trajectory::getAx(double current_time){
    //ROS_INFO("current_time= %0.4f", current_time);
    std::vector<float> profile = splineProfile(current_time, ax);
    return profile.at(2);
}
